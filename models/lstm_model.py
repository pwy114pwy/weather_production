...
class LSTMModel:
    ...
    def train(self, X_train, y_train, X_val, y_val, epochs=50, batch_size=32):
        ...
        callbacks = [
            ...
            ModelCheckpoint(
                filepath='d:\\nature-disaster-prediction\\models\\best_model.keras',
                monitor='val_recall',
                mode='max',
                save_best_only=True
            )
    ...
    def predict_future(self, X, days=7):
        ...
        for _ in range(days):
            ...
            # 计算历史数据的趋势特征
            trend_prediction = self._calculate_trend_prediction(current_sequence)
            future_predictions.append(trend_prediction)
            
            # 更新序列，将新预测添加到末尾，去掉最前面的时间步
            current_sequence = np.vstack([current_sequence[1:], trend_prediction])
...
if __name__ == "__main__":
    ...
    for label, count in zip(unique, counts):
        ...
        print(f"{label_names[label]}: {count} ({count/len(risk_labels)*100:.1f}%)")
    ...
    # 划分数据集
    print("\n划分训练集、验证集和测试集...")
    X_train, y_train, X_val, y_val, X_test, y_test = processor.split_data(sequences, targets)
