...
class LSTMWeatherModel:
    ...
    def _build_seq2seq_model(self):
        ...
        # 编码器LSTM层
        encoder_lstm1 = LSTM(units=64, return_sequences=True, activation='relu')(encoder_inputs)
        encoder_lstm1 = Dropout(0.2)(encoder_lstm1)
        
        encoder_lstm2 = LSTM(units=64, return_sequences=True, activation='relu')(encoder_lstm1)
        encoder_lstm2 = Dropout(0.2)(encoder_lstm2)
        
        encoder_lstm3 = LSTM(units=64, return_sequences=True, activation='relu')(encoder_lstm2)
        encoder_outputs = Dropout(0.2)(encoder_lstm3)
        
        # 解码器部分
        decoder_inputs = RepeatVector(self.pred_days)(encoder_lstm3[:, -1, :])
        
        # 解码器LSTM层
        decoder_lstm = LSTM(units=64, return_sequences=True, activation='relu')(decoder_inputs)
        ...
        outputs = TimeDistributed(Dense(self.output_features, activation='linear'))(merged)
    ...
    def _trend_based_prediction(self, X, days=7):
        ...
        for _ in range(days):
            ...
            current_sequence = np.concatenate([current_sequence[1:], next_step_reshaped], axis=0)
