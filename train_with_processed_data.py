...
def train_model_with_processed_data(data_file_path):
    ...
    try:
        ...
        if has_weather_targets:
            ...
            print("数据加载成功！")
            print(f"训练集形状: X={X_train.shape}, y={y_train.shape}, y_weather={y_weather_train.shape}")
            print(f"验证集形状: X={X_val.shape}, y={y_val.shape}, y_weather={y_weather_val.shape}")
        else:
            print("数据加载成功！")
        ...
        num_classes = len(np.unique(np.concatenate([y_train, y_val, y_test])))  # 自动检测类别数量
        ...
        if has_weather_targets:
            ...
            # 天气预测模型输入形状与风险预测模型相同
            weather_model = LSTMWeatherModel(input_shape=input_shape, output_features=X_train.shape[2])
            ...
            weather_model_path = os.path.join(model_dir, "final_weather_model.keras")
