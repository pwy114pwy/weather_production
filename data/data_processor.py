...
class DataProcessor:
    ...
    def preprocess(self, df):
        ...
        scaled_df = pd.DataFrame(
            scaled_features, columns=feature_cols, index=df.index)
    ...
    def create_sequences(self, df):
        ...
        for i in range(len(df) - self.seq_length - self.pred_length + 1):
            ...
            seq_target = df[target_col].iloc[i +
                                             self.seq_length+self.pred_length-1]

            # 未来多步的气象特征（用于Seq2Seq模型训练）
            future_weather = df[feature_cols].iloc[i +
                                                   self.seq_length:i+self.seq_length+self.pred_length].values
    ...
    def save_processed_data(self, data_dict, save_path):
        ...
        # 保存归一化器
        import joblib
        scaler_path = save_path.replace('.npz', '_scaler.pkl')
    ...
    def load_processed_data(self, load_path):
        ...
        # 加载归一化器
        import joblib
        scaler_path = load_path.replace('.npz', '_scaler.pkl')
        try:
            loaded_scaler = joblib.load(scaler_path)
            print(f"归一化器已从 {scaler_path} 加载")
        except FileNotFoundError:
            print(f"警告: 找不到归一化器文件 {scaler_path}，请确保在训练时一同保存")
            loaded_scaler = None
        ...
        # 添加天气预测相关的数据（如果存在）
        weather_keys = ['y_weather_train', 'y_weather_val', 'y_weather_test']
    ...
    def get_noaa_sequences(self, city_name, api_key, start_time=None, end_time=None):
        ...
        """
        # 获取预处理后的数据
        raw_df = self.load_noaa_data(
            city_name, api_key, start_time, end_time)['raw_df']
...
if __name__ == "__main__":
    ...
    # 划分数据集
    X_train, y_train, y_weather_train, X_val, y_val, y_weather_val, X_test, y_test, y_weather_test = processor.split_data(
        sequences, targets, weather_targets)
