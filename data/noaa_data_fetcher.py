...
class NOAAMeteorologicalDataFetcher:
    ...
    def fetch_weather_data(self, city_name, start_date, end_date):
        ...
        try:
            ...
            # 返回模拟数据
            df = self._get_mock_data(city_name, hours)
    ...
    def _process_noaa_data(self, df):
        ...
        # 转换为宽格式
        df_wide = df.pivot(index="datetime", columns="datatype", values="value")
        ...
        if "降雨量" in df_wide.columns:
            df_wide["持续时间"] = df_wide["降雨量"].apply(lambda x: 10 if x > 0 else 0)
        else:
            df_wide["降雨量"] = 0
            df_wide["持续时间"] = 0
